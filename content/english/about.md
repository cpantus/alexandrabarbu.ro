---
title: "About Me - Psychotherapist Alexandra Barbu"
description: "15+ years of experience in psychotherapy. Specialization in CBT, EMDR, Mindfulness. Empathetic and evidence-based approach."
layout: "flexible"

sections:
  - type: "hero-about"
  - type: "credentials-education"
  - type: "scientific-approach"

# ============================================================================
# 1. HERO SECTION
# ============================================================================

hero_about:
  kicker: "WHO I AM"
  title: "Alexandra *Barbu*"
  subtitle: "Clinical psychologist with an integrative, evidence-based approach"
  description: "With over 10 years of experience and thousands of therapy hours delivered, I guide each person to discover their own inner resources for healing. I believe in the power of empathy and scientifically validated methods."
  paragraphs:
    - "I am a clinical psychologist with over 10 years of experience in individual and couples therapy. My mission is to help you find clarity, balance, and the power to face your challenges."
    - "I have worked with dozens of clients from diverse backgrounds, and each therapeutic journey has taught me that there is no universal recipe for healing. My approach is personalized, combining scientifically validated methods with deep empathy."
    - "I believe that every person has within them the resources necessary for change. My role is to guide you in discovering them, in a safe, confidential, and judgment-free space."
  quote:
    text: "Every person has within them the resources necessary for healing. My role is to guide you in discovering them."
    author: "Alexandra Barbu"
  image: "images/about/alexandra-portrait.jpg"


# ============================================================================
# 2. CREDENTIALS & EDUCATION
# ============================================================================

credentials_education:
  title: "Education and *Credentials*"
  subtitle: "PROFESSIONAL CREDENTIALS"
  description: "Continuous education is fundamental to providing the best therapeutic services. Here is my professional training."
  credentials:
    - icon: "graduation-cap"
      title: "University Studies"
      items:
        - "Doctorate in Clinical Psychology, University of Bucharest"
        - "Master's in Cognitive Behavioral Therapy, UBB Cluj"
        - "Bachelor's in Psychology, University of Bucharest"
      variant: "primary"
    - icon: "user-md"
      title: "Certifications"
      items:
        - "Member of the College of Psychologists of Romania (CPR)"
        - "EMDR Europe Certificate - Practitioner Level"
        - "Member of the European Association for Psychotherapy"
      variant: "secondary"
    - icon: "book"
      title: "Specializations"
      items:
        - "Cognitive Behavioral Therapy (CBT) - 300 hours"
        - "Dialectical Behavior Therapy (DBT) - 200 hours"
        - "Acceptance and Commitment Therapy (ACT)"
      variant: "coral"

# ============================================================================
# 3. SCIENTIFIC APPROACH
# ============================================================================

scientific_approach:
  label: "SCIENTIFIC APPROACH"
  title: "Where Science Meets *Empathy*"
  description: "My approach combines the rigor of scientifically validated methods with the warmth of human empathy. Each session is personalized to your unique needs, in a safe and judgment-free space."
  button_text: "Discover My Complete Methodology"
  button_url: "/en/approach/"
  button_variant: "primary"
  features:
    - icon: "brain"
      title: "Neuroplasticity"
      description: "The brain can create new neural connections at any age, allowing for profound and lasting changes in thinking and behavior."
      variant: "primary"
    - icon: "microscope"
      title: "Clinical Validation"
      description: "I use exclusively scientifically validated methods, with proven effectiveness in international studies and recognized clinical guidelines."
      variant: "secondary"
    - icon: "heart"
      title: "Therapeutic Alliance"
      description: "An authentic therapeutic relationship, based on trust and empathy, is the strongest predictor of therapy success."
      variant: "coral"
    - icon: "book-open"
      title: "Psychoeducation"
      description: "Understanding psychological processes and the mechanisms of change empowers you in your journey of healing and personal development."
      variant: "sage"



# ============================================================================
# 7. CTA SECTION
# ============================================================================

cta_standard:
  title: "Let's Get to Know Each Other"
  description: "Schedule a free 30-minute consultation to see if we can work together. No obligations, completely confidential."
  button_text: "Schedule a Consultation"
  button_url: "https://cal.com/gigi-frana-uvymh4/15min"
  button_variant: "secondary"
---
