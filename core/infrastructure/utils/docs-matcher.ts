...
export function loadDocsIndex(projectRoot: string): DocsIndex | null {
  const indexPath = path.join(projectRoot, '.claude', 'docs', 'docs-index.json');
  ...
  for (const [docName, metadata] of Object.entries(docsIndex.docs)) {
    ...
    if (score > 0) {
      matches.push({
        ...
        docPath: `.claude/docs/${metadata.file}`,
        score
      });
    }
  }
...
export function loadDocContent(
  ...
  const docPath = path.join(projectRoot, '.claude', 'docs', fileName);
