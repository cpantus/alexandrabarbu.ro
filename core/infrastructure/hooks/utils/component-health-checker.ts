...
const BASE_DIR = path.resolve(process.cwd(), '.claude');
...
function isFileInRegistry(filePath: string): boolean {
  ...
  // Check docs-index.json (now in .claude/)
  try {
    const docsIndexPath = path.resolve(process.cwd(), '.claude', 'docs-index.json');
    const docsIndex = JSON.parse(
      fs.readFileSync(docsIndexPath, 'utf-8')
    );
