...
function loadTierConfig(): TierConfig {
  ...
  if (!fs.existsSync(configPath)) {
    // Tier config is optional - return minimal config if not found
    tierConfig = {
      version: '1.0.0',
      tiers: {
        '1': { name: 'Core', description: 'Always visible', visibility: 'always', keywords: [] }
      },
      suggestionRules: {},
      visibilityMatrix: {
        productionMode: {},
...
function loadSystemRegistry(): SystemRegistry {
  ...
  if (!fs.existsSync(registryPath)) {
    // System registry is optional - return empty registry if not found
    // This allows the hook to work without tier filtering
    systemRegistry = {
      version: '1.0.0',
      agents: [],
      patterns: [],
